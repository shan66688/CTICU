...
```{r remove ethnicity }
back_logistic_fit_2 <- glm(formula= period ~ IHV_24 +BMI +PMM +PRM + PPV+ Reop+Urgent +Sug_Dur +Pri_MI +smoker+lung+ PFT +RFD + PAD+ POBPT+ TBU+ XCT+ CPBT, family = binomial(link = "logit"), data = glmdata)
summary(back_logistic_fit_2)
```

```{r remove reop }
back_logistic_fit_3 <- glm(formula= period ~ IHV_24 + Ethnicity +BMI +PMM +PRM + PPV +Urgent +Sug_Dur +Pri_MI +smoker+lung+ PFT +RFD + PAD+ POBPT+ TBU+ XCT+ CPBT, family = binomial(link = "logit"), data = glmdata)
summary(back_logistic_fit_3)
```

```{r remove PRM }
back_logistic_fit_4 <- glm(formula= period ~ IHV_24  + Ethnicity+BMI +PMM + PPV+Urgent +Sug_Dur +Pri_MI +smoker+lung+ PFT +RFD + PAD+ POBPT+ TBU+ XCT+ CPBT, family = binomial(link = "logit"), data = glmdata)
summary(back_logistic_fit_4)
```

```{r remove BMI }
back_logistic_fit_5 <- glm(formula= period ~ IHV_24 + Ethnicity +PMM + PPV+Urgent +Sug_Dur +Pri_MI +smoker+lung+ PFT +RFD + PAD+ POBPT+ TBU+ XCT+ CPBT, family = binomial(link = "logit"), data = glmdata)
