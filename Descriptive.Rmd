...
desdata <- rawdata %>% 
  ...
  mutate(IHV_24 = IHV, ICU_LOS = ICU_LOS/24)
...
glmdata$Ethnicity[glmdata$Ethnicity == "Hawaiian"] <- "Other"
glmdata$Ethnicity[glmdata$Ethnicity == "American Indian"] <- "Other"
...
desdata <- desdata %>% mutate( IHV_16 = IHV)
desdata$IHV_16[desdata$IHV>=16] = NA 

ggg <- ggplot(desdata, aes(desdata$period, desdata$IHV_16))
ggg + geom_boxplot(varwidth=T, fill="plum") + 
    labs(title="Box plot", 
...
desdata$IHV_16[desdata$period == "P1"] %>% na.omit() %>% length()
summary(desdata$IHV_16[desdata$period == "P1"])
desdata$IHV_16[desdata$period == "P1"] %>% na.omit() %>% sd()
desdata$IHV_16[desdata$period == "P2"] %>% na.omit() %>% length()
summary(desdata$IHV_16[desdata$period == "P2"])
desdata$IHV_16[desdata$period == "P2"] %>% na.omit() %>% sd()
desdata$IHV_16 %>% na.omit() %>% length()
summary(desdata$IHV_16)
desdata$IHV_16 %>% na.omit() %>% sd()

var.test(desdata$IHV_16[desdata$period == "P2"], desdata$IHV_16[desdata$period == "P1"])
t.test(x= desdata$IHV_16[desdata$period == "P1"], y = desdata$IHV_16[desdata$period == "P2"], paired = FALSE, var.equal = TRUE, conf.level = 0.95)
```
...
plotdata <- desdata %>% mutate(OREhour = as.factor(OREhour))
g <- ggplot(plotdata, aes(plotdata$OREhour, plotdata$IHV_24))
...
par(mfrow=c(1,1))
hist(desdata$IHV[desdata$period == "P1"], main = "Histogram of Ventilation Duration in Day Shift", xlab = "Ventilation Duration for Patients Extubated within 24 hours")
hist(desdata$IHV[desdata$period == "P2"], main = "Histogram of Ventilation Duration in Night Shift", xlab = "Ventilation Duration for Patients Extubated within 24 hours")
...
```{r}
test_logistic_fit <- glm(formula= period ~ IHV_24 + Sug_Dur + smoker +Ethnicity +XCT  , family = binomial(link = "logit"), 
    data = glmdata)
...
full_logistic_fit <- glm(formula= period ~ IHV_24+Ethnicity + PPV   +smoker+ PFT  + XCT+ CPBT, family = binomial(link = "logit"), data = glmdata)
...
```{r}
back_logistic_fit <- glm(formula= period ~ IHV_24 + PPV   +smoker+ PFT  + PAD+ XCT+ CPBT, family = binomial(link = "logit"), data = glmdata)
summary(back_logistic_fit)
exp(back_logistic_fit$coefficients)
```
...
```{r backward selection}
full_logistic_fit <- glm(formula= period ~ IHV_24+Ethnicity+ age +BMI +PMM +PRM + PPV+ Reop+Urgent +Sug_Dur +Pri_MI +smoker+lung+ PFT +RFD + PAD+ POBPT+ TBU+ XCT+ CPBT, family = binomial(link = "logit"), data = glmdata)
summary(full_logistic_fit)
...
```
```{r remove PPV }
back_logistic_fit_14 <- glm(formula= period ~ IHV_24+Ethnicity +PMM  +Sug_Dur +smoker+ PFT , family = binomial(link = "logit"), data = glmdata)
summary(back_logistic_fit_14)
```
```{r remove PMM }
back_logistic_fit_15 <- glm(formula= period ~ IHV_24+Ethnicity   +Sug_Dur +smoker+ PFT , family = binomial(link = "logit"), data = glmdata)
summary(back_logistic_fit_15)
```

```{r remove Sug_dur }
back_logistic_fit_16 <- glm(formula= period ~ IHV_24+Ethnicity +smoker+ PFT , family = binomial(link = "logit"), data = glmdata)
summary(back_logistic_fit_16)
```
```{r Final model}
final_logistic_fit <- back_logistic_fit_15
summary(final_logistic_fit)
final_logistic_fit$coefficients

IHV_24_OR <- exp(final_logistic_fit$coefficients[2]*5) %>% print()
rest_OR <- exp(final_logistic_fit$coefficients[3:10]) %>% print()
exp(0.02094841)
```
...
par(mfrow=c(1,1))
hist(desdata$Hos_LOS[desdata$period == "P1"], main = "Histogram of Ventilation Duration in Day Shift", breaks = 24, xlab = "Hospital LOS")
hist(desdata$Hos_LOS[desdata$period == "P2"], main = "Histogram of Ventilation Duration in Night Shift",breaks = 12, xlab = "Hospital LOS")

```

```{r Hos_LOS logistic}
Hos_LOS_fit <- glm(formula= period ~ Hos_LOS, family = binomial(link = "logit"), 
    data = glmdata)
summary(Hos_LOS_fit)
exp(0.009537*10)
```
...
```{r ICU_LOS logistic}
ICU_LOS_fit <- glm(formula= period ~ ICU_LOS, family = binomial(link = "logit"), 
    data = glmdata)
summary(ICU_LOS_fit)
exp(-0.01584 * 5)
```
...
fisher.test(glmdata$Reintub,glmdata$period)
```

```{r Reintub logistic}
Reintub_fit <- glm(formula= period ~ Reintub, family = binomial(link = "logit"), 
    data = glmdata)
summary(Reintub_fit)
exp( -0.45347)
```

```{r Post_pneu descriptive}
#desdata$Post_pneu[desdata$Post_pneu == "Not documented"] <- NA
length(desdata$Post_pneu)
sum(is.na(desdata$Post_pneu))
sum(is.na(desdata$Post_pneu)) / length(desdata$Post_pneu)
table(desdata$Post_pneu)

#table(desdata$Post_pneu) / length(desdata$Post_pneu)
paste(round(100*(table(desdata$Post_pneu) / 861), 2), "%", sep="")

length(desdata$Post_pneu[desdata$period == "P1"])
length(desdata$Post_pneu[desdata$period == "P2"])

sum(is.na(desdata$Post_pneu[desdata$period == "P1"]))
sum(is.na(desdata$Post_pneu[desdata$period == "P2"]))

table(desdata$Post_pneu[desdata$period == "P1"]) 
table(desdata$Post_pneu[desdata$period == "P2"]) 

#table(desdata$Post_pneu[desdata$period == "P1"]) / length(desdata$Post_pneu[desdata$period == "P1"])
paste(round(100*(table(desdata$Post_pneu[desdata$period == "P1"]) / length(desdata$Post_pneu[desdata$period == "P1"])), 2), "%", sep="")

#table(desdata$Post_pneu[desdata$period == "P2"]) / length(desdata$Post_pneu[desdata$period == "P2"])
paste(round(100*(table(desdata$Post_pneu[desdata$period == "P2"]) / length(desdata$Post_pneu[desdata$period == "P2"])), 2), "%", sep="")


fisher.test(desdata$Post_pneu,desdata$period)
```
...
fisher.test(glmdata$Re_ICU ,glmdata$period)
```

```{r Re_ICU logistic}
Re_ICU_fit <- glm(formula= period ~ Re_ICU, family = binomial(link = "logit"), 
    data = glmdata)
summary(Re_ICU_fit)
exp( 0.20930)
```
...
```{r Mor_disc logistic}
Mor_disc_fit <- glm(formula= period ~ Mor_disc, family = binomial(link = "logit"), 
    data = glmdata)
summary(Mor_disc_fit)
exp( -12.91105)
```
